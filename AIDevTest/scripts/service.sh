#!/usr/bin/env bash
# AIDevTest 服务管理器 — phantom --dev-test 包装 worker
set -euo pipefail
PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SERVICE_NAME="ai-devtest"
SERVICES=(worker)
sig_worker="ai_devtest.agent"
...
start_worker() {
  ...
  PYTHONUNBUFFERED=1 nohup .venv/bin/python -m ai_devtest.agent \
    >"$(log_file worker)" 2>&1 &
