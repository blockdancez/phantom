...
- **Dev**：Generator 在 compaction 长会话里一次实现整组 feature，写单元测试（核心覆盖率 100%），跑静态检查（ruff/eslint/tsc 等）**0 error**，自修复无限
- **Code-review**：跨模型 reviewer 审 diff（placeholder / mock / 日志规范 / API 契约 / 安全红线），**shell 侧 grep 兜底**（TODO / FIXME / console.log / 硬编码端口 / 硬编码凭据）
- **Deploy**：Generator 写 `scripts/start-backend.sh`（+ `start-frontend.sh` 如有），**shell 用 `nohup` 本地启动 + 等端口 + curl smoke**，2 次自试后失败回 dev。前后端预分配独立端口避免多项目冲突，运行时日志在 `.phantom/runtime/`
- **Test**：跨模型 tester 用 Playwright MCP + curl 跑所有端点所有场景，按 rubric 打分。**累积测试所有已完成 feature**（防回归），总分 ≥ 90 过关
...
<project>/
  .phantom/
    ...
    port.backend            # 后端预分配端口
    port.frontend           # 前端预分配端口
    runtime/
      backend.pid           # 运行中后端 PID
  ...
  scripts/
    start-backend.sh        # deploy phase 写的启动脚本
