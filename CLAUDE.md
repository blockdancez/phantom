...
- **主循环按 feature 走**：一次一个 feature，按 plan 第 5 节列表顺序；每个 feature 走完 `dev → code-review → deploy → test` 一整圈才进下一个
...
1. **`run_plan_phase`** — R1 planner 起草 9 节 plan（`_plan_has_all_9_sections` 校验）→ R2 plan-reviewer 跨模型审 rubric（无产出也 OK，降级为"无意见"）→ R3 planner 根据 comments 修订 → `cp plan.md plan.locked.md` 冻结 → 解析 feature 列表（`list_features_from_plan`，至少 5 个）
2. **`run_dev_phase`** — 单次 dev round。注入 `FEATURE` 占位符，走 `ai_run generator`（compaction），校验 `changelog.md` 新增 `## Iteration N` 条目，没增加则触发补救 round。完成后归档并清除 return-packet
...
5. **`run_test_phase`** — 单次 test round。走 `ai_run tester`（跨模型），tester 用 Playwright MCP + curl 跑所有端点所有场景，按 rubric 打分写 `test-report-iter<N>.md`，shell 提取总分，≥80 pass，否则 fail 并写 return-packet

### Feature 迭代（`phantom.sh::run_feature_sprint`）

```
for each feature in plan.locked.md §5:
  for round in 1..max_rounds:
    run_dev_phase
    run_code_review_phase  (reject → continue loop)
    run_deploy_phase       (fail → continue loop)
    run_test_phase         (<80 → continue loop)
    if round >= min_rounds: break (feature done)
    else: shell 写"强制打磨"return-packet 继续 loop
```

每 feature `min_rounds=2`（`--fast` 时 `=1`），`max_rounds=6`。达 max strict 模式 exit 1，默认 `mark_forced_feature` 继续。

### Handoff 文件（`.phantom/` 下）

| 文件 | 作用 |
|---|---|
| `state.json` | Phase 状态 + `current_feature_index` + `test.forced_features[]` |
| `plan.md` | Phase 1 工作稿 |
...
- Plan 的 9 节结构：`_plan_has_all_9_sections` grep
...
- `PHANTOM_STRICT=1` / `--strict`：feature 达到 max_rounds 直接 `exit 1`
- `PHANTOM_FAST=1` / `--fast`：`DEV_MIN_ROUNDS=1`（跳过"强制打磨"轮）

## 修改指引

- **改工作流顺序 / 轮数 / gate**：只动 `lib/phases.sh`（各 phase 函数）或 `phantom.sh::run_feature_sprint`，不要把逻辑塞进 prompt
...
- **改完成判定**：
  - plan phase → 改 `_plan_has_all_9_sections` 的 expected 数组
