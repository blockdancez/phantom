...
依赖：`claude` 或 `codex` CLI、`jq`、`python3`、`curl`、可选 `postgres` MCP、可选 `playwright`（前端 E2E）。不再需要 Docker——deploy 改为本地启动进程。
...
4. **`run_deploy_phase`** — 单次 deploy round，内部自试 2 次。AI 写 `scripts/start-backend.sh`（+ `start-frontend.sh` 如有前端），shell 侧：kill 旧 PID → `nohup` 启动新进程 → 等端口就绪（60s）→ smoke test（所有 API 端点 happy path curl 非 5xx）。成功后进程**常驻**（不清理），日志落到 `.phantom/runtime/{backend,frontend}.log`。失败 2 次后写 return-packet 回 dev
5. **`run_test_phase`** — 单次 test round。走 `ai_run tester`（跨模型），tester 生成 Playwright 测试脚本（`.playwright/tests/`）并执行 + curl 跑所有端点所有场景，按 rubric 打分写 `test-report-iter<N>.md`。Shell 解析 `.playwright/results.json` 日志输出 pass/fail 兜底，提取总分 ≥90 pass，否则 fail 并写 return-packet。可用 Chrome DevTools MCP 辅助调试失败场景
...
| `port.backend` / `port.frontend` | 预分配的后端 / 前端端口（`socket.bind(0)` 拿到，避免并行项目冲突） |
| `port` | 向后兼容：等同 `port.backend` |
