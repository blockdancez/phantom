#!/usr/bin/env bash
# 前台启动 ai-design worker：控制台 + 文件双路日志（由 Python logging 写入）
# 默认日志路径：/Users/lapsdoor/phantom/logs/ai-design.log
...
echo "[run.sh] AI_DESIGN_LOG_DIR=$AI_DESIGN_LOG_DIR"
echo "[run.sh] 控制台与日志文件均由 Python logging 同步写入；Ctrl+C 退出"
echo "------------------------------------------------------------"

exec .venv/bin/python -m ai_design.agent
